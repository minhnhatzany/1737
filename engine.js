...
function pushBattleLedger(state, entry) {
...
function collectWarControlStats(state) {
  const regions = getAllRegions();
  const ids = [];
  for (const r of regions) {
    for (const ph of Object.values(r.phu || {})) {
 ...
 */
function recordWarRegionalIncident(state, regionId, regionName, incident) {
  if (isWarTruceActive(state)) return;
...
function flushWarRegionalDigestForYear(state, yearClosing) {
  ensureWarRegionalDigestScratch(state);
...
function isRecentWarFrontPulse(state, maxDays = 48) {
...
function hasRebelHeldMajorFrontHuyen(state) {
  const regions = getAllRegions();
  for (const r of regions) {
    for (const ph of Object.values(r.phu || {})) {
      for (const h of Object.values(ph.huyen || {})) {
...
function isWarStillRaging(state) {
...
function updateWarFrontControl(state) {
  if (!state._huyenControl) state._huyenControl = {};
  const regions = getAllRegions();
  for (const r of regions) {
    for (const ph of Object.values(r.phu || {})) {
...
function ensureWarAiState(state) {
...
function ensureAdvancedWarState(state) {
...
function isWarTruceActive(state) {
  const ym = currentYmSerial(state);
  return (state?._warAi?.truceUntilYm || 0) >= ym;
}

function ensureWarEconomyByHuyen(state, entries) {
  ensureAdvancedWarState(state);
...
function updateMonthlyWarEconomyByHuyen(state, entries) {
...
function createWarConvoy(state, side, fromEntry, toEntry) {
  ensureAdvancedWarState(state);
...
function planMonthlyWarConvoys(state, entries) {
  ensureAdvancedWarState(state);
...
function tickWarConvoysDaily(state, entries) {
  ensureAdvancedWarState(state);
...
function tryMonthlyWarTruce(state) {
  ensureAdvancedWarState(state);
  if (isWarTruceActive(state)) return;
  const tri = getFactionStore(state, Faction.TRIEU_DINH);
  const nq = getFactionStore(state, Faction.NGHIA_QUAN);
...
function tickWarObjectivesMonthly(state, entries) {
  ensureAdvancedWarState(state);
...
function pushYearlyWarReplay(state, prevYear) {
...
function getAllWarHuyenEntries(state) {
  const out = [];
  const regions = getAllRegions();
  for (const r of regions) {
...
function estimateFrontlineStrength(state, entry, faction) {
...
function strategicAiRecruitWarChest(state, faction) {
  const store = getFactionStore(state, faction);
  if (!store) return;
...
function tickStrategicWarAi(state) {
  ensureWarAiState(state);
  if (isWarTruceActive(state)) return;
...
function tickLiveBattles(state) {
...
function processMonthlyWarEconomyAI(state) {
